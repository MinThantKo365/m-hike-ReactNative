import React from 'react';
import { 
    View, 
    Text, 
    ScrollView, 
    StyleSheet, 
    TouchableOpacity,
    Linking, 
    Alert, 
} from 'react-native';


export default function HikeDetails({ 
    hike, 
    onBack, 
}) {
    

    const renderStat = (label, value, unit = '') => {

        if (value === null || value === undefined || value === '') return null;


        let displayValue = value;
        if (label === 'Parking') {
            displayValue = value === 'Available' ? '✅ Yes' : (value === 'Unavailable' ? '❌ No' : value);
        }
        
        return (
            <View style={styles.statItem}>
                <Text style={styles.statLabel}>{label}</Text>
                <Text style={styles.statValue}>{displayValue} {unit}</Text>
            </View>
        );
    };

 
    const handleOpenMap = () => {
        
        const locationQuery = hike.location.replace(/ /g, '+'); 
        const url = `http://maps.google.com/maps?q=${locationQuery}`; 

        Linking.canOpenURL(url).then(supported => {
            if (supported) {
                Linking.openURL(url);
            } else {
                Alert.alert("Map Error", `Cannot open map for this location.`);
            }
        }).catch(err => console.error('An error occurred trying to open map:', err));
    };

    return (
        <View style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={styles.scrollContainer}>

                <View style={[styles.baseCard, styles.mainInfoCard]}>
                    <Text style={styles.hikeName}>{hike.name}</Text>
                    <Text style={styles.hikeLocation}>{hike.location}</Text>
                    <Text style={styles.detailText}>
                        <Text style={styles.label}>Logged Date:</Text> {hike.date}
                    </Text>
                </View>


                <View style={[styles.baseCard, styles.statsGroup]}>
                    {renderStat("Length", hike.length, 'km')}
                    {renderStat("Difficulty", hike.difficulty)}
                    {renderStat("Parking", hike.parking)}
                    {renderStat("Time", hike.estimatedTime, 'min')}
                    {renderStat("Trail Type", hike.trailType)}
                </View>


                {hike.description ? (
                    <View style={[styles.baseCard, styles.descriptionCard]}>
                        <Text style={styles.descriptionLabel}>Description</Text>
                        <Text style={styles.descriptionText}>{hike.description}</Text>
                    </View>
                ) : null}

       
                <View style={{ height: 20 }} />

            </ScrollView>

            <View style={styles.bottomBar}>
                 <TouchableOpacity 
                    style={[styles.actionButton, styles.mapButton]} 
                    onPress={handleOpenMap}
                >
                    <Text style={styles.buttonText}>Open in Map</Text>
                </TouchableOpacity>
                
                <TouchableOpacity 
                    style={[styles.actionButton, styles.backButton]} 
                    onPress={onBack}
                >
                    <Text style={styles.buttonText}>← Back to All Hikes</Text>
                </TouchableOpacity>
            </View>
        </View>
    );
}

const styles = StyleSheet.create({
    scrollContainer: {
        padding: 10,
        backgroundColor: '#f4f4f4',
    },

    baseCard: {
        backgroundColor: '#fff',
        borderRadius: 12,
        padding: 20,
        marginBottom: 10,
        shadowColor: '#000',
        shadowOffset: { width: 0, height: 2 },
        shadowOpacity: 0.1,
        shadowRadius: 4,
        elevation: 3,
    },

    mainInfoCard: {
        borderLeftWidth: 5,
        borderLeftColor: '#4CAF50', 
    },
    hikeName: {
        fontSize: 26,
        fontWeight: 'bold',
        color: '#333',
        marginBottom: 4,
    },
    hikeLocation: {
        fontSize: 18,
        color: '#555',
        marginBottom: 10,
    },
    label: {
        fontWeight: '700',
        color: '#333',
    },
    detailText: {
        fontSize: 14,
        color: '#666',
    },

    statsGroup: {
        flexDirection: 'row',
        flexWrap: 'wrap',
        justifyContent: 'space-between',
        marginTop: 10,
    },
    statItem: {
        width: '47%', 
        marginBottom: 15,
        paddingHorizontal: 5,
    },
    statLabel: {
        fontSize: 12,
        fontWeight: '600',
        color: '#999',
        textTransform: 'uppercase',
    },
    statValue: {
        fontSize: 16,
        fontWeight: 'bold',
        color: '#444',
    },

    descriptionCard: {
        minHeight: 100,
        marginTop: 10,
    },
    descriptionLabel: {
        fontSize: 16,
        fontWeight: '700',
        color: '#333',
        marginBottom: 5,
        borderBottomWidth: 1,
        borderBottomColor: '#eee',
        paddingBottom: 5,
    },
    descriptionText: {
        fontSize: 14,
        color: '#666',
        lineHeight: 22,
    },

    bottomBar: {
        flexDirection: 'row', 
        justifyContent: 'space-between',
        paddingHorizontal: 10,
        paddingTop: 10,
        paddingBottom: 20, 
        backgroundColor: '#fff',
        borderTopWidth: 1,
        borderTopColor: '#eee',
    },
    actionButton: {
        flex: 1,
        paddingVertical: 14,
        borderRadius: 8,
        alignItems: 'center',
        marginHorizontal: 5,
        shadowColor: '#000',
        shadowOffset: { width: 0, height: 2 },
        shadowOpacity: 0.2,
        shadowRadius: 2,
        elevation: 5,
    },
    mapButton: {
        backgroundColor: '#f1c40f', // Yellow/Gold for Map
    },
    backButton: {
        backgroundColor: '#3498db', // Blue for Back
    },
    buttonText: {
        color: 'white',
        fontWeight: 'bold',
        fontSize: 16,
    }
});